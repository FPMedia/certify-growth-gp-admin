...
Stored but not rendered in the live product (`paragraph2`, company **Next steps**) remain editable with an explicit “not currently shown” note. **CEO question labels** (`ceoQuestionLabel`, `ceoQuestionLabel2`) are **prompts only** for per-element company notes (Q1 / Q2) in the web **Roadmap notes** panel (a third **General note** uses a fixed label). Fixed headings **Why This Matters to Our Growth** and **What We Need to Strengthen** appear above Q1/Q2 prompts in the app UI. Changing a label does not alter saved note text; the new prompt wording appears on reports after recompile or a new completion. The note bodies themselves are live in identity and do not require recompile.
